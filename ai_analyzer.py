"""
ai_analyzer.py v4 — Gemini API 호출 전담
"""
...
def _call_gemini(prompt: str, json_mode: bool = True, timeout: int = 60) -> tuple:
    ...
    try:
        ...
        return None, f"JSON 파싱 오류: AI가 JSON 문법을 어겼습니다.\n[... {snippet} ...]\n재분석 버튼을 눌러 다시 시도해 주세요."
...
def diagnose_gallery_ai(gallery_name: str, top_words: list, subtype_id: str, daily_avg: float) -> tuple:
...
def _format_posts(post_data: list) -> str:
    ...
    for idx, post in enumerate(post_data):
        ...
        lines.append(
            f"[{idx+1}] 제목: {post.get('title','')} | 작성일: {post.get('date','')} | "
            f"URL: {post.get('post_url','')} | 본문: {body[:600]}\n"
        )
    return "".join(lines)
...
def analyze_gallery(gallery_id: str, game_name: str, subtype_id: str,
    ...
    prompt    = build_main_analysis_prompt(
        gallery_id=gallery_id, game_name=game_name, subtype_label=sl, subtype_desc=sd,
        subtype_focus=sf, analysis_focus_label=focus_lbl,
        post_data_text=post_text, analysis_days=analysis_days, 
        total_posts=len(all_metas), concept_posts=sum(1 for m in all_metas if m.get("is_concept")),
